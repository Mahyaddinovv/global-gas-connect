...
const BuilderManager = () => {
    ...
    try {
      ...
      try {
        await createAuditLog({
          action: "Updated visual page builder settings",
          section: "builder",
        });
      } catch {
        auditFailed = true;
  ...
  return (
    <Card className="border-slate-800 bg-slate-900/80">
      <CardHeader>
        <CardTitle className="text-sm font-medium text-slate-100">Visual page builder</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {isLoading && <p className="text-xs text-slate-400">Loading builder blocks...</p>}
            ...
            >
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div className="flex items-center gap-3">
                  <div className="rounded-md border border-slate-700 p-2 text-slate-300">
                    <GripVertical className="h-4 w-4" />
                  </div>
                  <div>
                    <div className="font-medium text-slate-100">{blockTitles[block.section]}</div>
                    <div className="text-xs text-slate-400">{block.section}</div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    size="icon"
                    variant="outline"
                    className="h-8 w-8 border-slate-700 bg-slate-900 text-slate-100 hover:bg-slate-800 hover:text-white"
                  ...
                  >
                    {block.visible ? <Eye className="h-4 w-4 text-sky-400" /> : <EyeOff className="h-4 w-4 text-slate-500" />}
                  </Button>
                </div>
              </div>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <div className="space-y-2">
                  <label className="block text-xs font-medium text-slate-300">Visible</label>
                  <div className="flex h-10 items-center rounded-md border border-slate-800 bg-slate-900 px-3">
                    <Switch
                      checked={block.visible}
                      onCheckedChange={(value) => updateBlock(block.id, { visible: value === true })}
                    />
                ...
                <div className="space-y-2">
                  <label className="block text-xs font-medium text-slate-300">Background type</label>
                  <Select
                    value={block.bg_type}
                    onValueChange={(value) => updateBlock(block.id, { bg_type: value as CmsBlockBackgroundType })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="space-y-2">
                  <label className="block text-xs font-medium text-slate-300">Alignment</label>
                  <Select
                    value={block.alignment}
                    onValueChange={(value) => updateBlock(block.id, { alignment: value as CmsBlockAlignment })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="space-y-2">
                  <label className="block text-xs font-medium text-slate-300">Padding top / bottom</label>
                  <Input
                    type="number"
                    min={0}
                    step={8}
                ...
                <Input
                  ...
                  placeholder={
                    block.bg_type === "gradient"
                      ? "linear-gradient(135deg, #0f4c81, #2db5c0)"
                      : "#ffffff or hsl(var(--background))"
                  }
                />
              </div>
            </div>
