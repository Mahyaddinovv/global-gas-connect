...
const UsersManager = () => {
  ...
  const loadUsers = async () => {
    setLoading(true);
    const { data, error } = await supabase.from("cms_users").select("id, email, role").order("created_at", {
      ascending: true,
    });
    if (error) {
      toast({
  ...
  const removeUser = async (id: string) => {
    ...
    if (error) {
      ...
      toast({
        title: "User removed",
        description: `${userToRemove.email} has been removed.`,
      });
    }
  };
  ...
  if (!isSuperadmin) {
    return (
      <Card className="border-slate-800 bg-slate-900/80">
        <CardHeader>
          <CardTitle className="text-sm font-medium text-slate-100">Users</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  return (
    <Card className="border-slate-800 bg-slate-900/80">
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2 rounded-md border border-slate-800/80 bg-slate-950/80 p-3">
          <div className="text-xs font-medium text-slate-200">Invite new user</div>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {users.map((u) => (
              <div
                key={u.id}
                className="flex flex-col gap-2 rounded-md border border-slate-800/80 bg-slate-950/80 p-3 text-sm md:flex-row md:items-center md:justify-between"
              >
                <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    variant="outline"
                    className="border-red-900/60 text-red-200 hover:bg-red-900/40 hover:text-red-50"
